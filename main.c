...
void LM35Kit_ADC_init() {
    ADMUX = 0x81 ;   //Channel 1(5 bits)&,VCC (2BIT) ,ADLDRA IS RIGHT (1BIT).
    //Note that the AVCC IN THE KIT is the one connected to the 5v and connected to capacitor TO AREF, SO WE HAVE TO TAKE VCC
...
ISR(ADC_vect) {
    ...
    if (data >= 2500)//Relay and LED will be turned on when the data is larger than 2500 otherwise thwy will turn off
    {
        setPINA(Relay);
        setPIND(LED);
